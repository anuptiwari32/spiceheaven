...
class BranchController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if($files=$request->file('menus')){
            foreach($files as $file){
                $image[]=Helpers::upload('branch_menu/', 'png', $file);
            }
        }
        else 
        {
        ...
        $branch->capacity = $request->coverage ? $request->coverage : 20;
        $branch->menu_image= json_encode($image);
        $branch->save();
